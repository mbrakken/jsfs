...
module.exports.load_inode = function load_inode(url, callback){
  ...
  var inode_fingerprint = sha1_to_hex(url);
...
// Use analyze to identify offset until non-zero data, grab just that portion to store
// In analyze we identified the data chunk offset and data block size
...
module.exports.wave_audio_offset = function wave_audio_offset(block, data, default_size){
  // block_byte_size most likely to be 4, but it'd be nice to handle alternate cases.
  // Essentially, we should use block["readUInt" + (block_byte_size * 8) + "LE"]() to scan the block.
  var block_byte_size = data.data_block_size;

  if (data.subchunk_id === "data" && block_byte_size === 4) {
    ...
    // Increment our offset by block_byte_size, since we're analyzing on the basis of it.
    // Keep blocks within the chunk intact.
    for (data_offset; (data_offset + block_byte_size) < block_length; data_offset = data_offset + block_byte_size) {
